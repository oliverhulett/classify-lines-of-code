...
## Format JSON
git ls-files | grep -iE '.json$' | while read -r; do
...
## Format YAML
git ls-files | grep -iE '.ya?ml$' | while read -r; do
...
##     # ... some very large, complex data literal.
## }  # yapf: disable
git ls-files | grep -iE '.py$' | while read -r; do
...
##             --option3 \
## # @formatter:on
	git ls-files | grep -iE '.(ba)?sh$' | while read -r; do
