...
def _discover(ctx):
    ...
    with ctx.cd(_PROJECT_DIR):
        _src_dirs = [
            x
            for x in ctx.run(
                "git ls-files -z src | xargs -0n 1 dirname | sort -u", hide="both", echo=False
            ).stdout.split()
...
def format(ctx, verbose=False):
    ...
    with ctx.cd(_PROJECT_DIR):
        ctx.run(
            "bash <(curl -sS https://bitbucket.org/oliverhulett/formatter/raw/master/run.sh) {} format --exclude docs/htmlcov/".format(
...
def check(ctx):
    ...
    if not os.path.exists(pychecker_file):
        ...
        for p in ("python2", "python2.6", "python2.7"):
...
@task(pre=(format, check, test, call(coverage_report, all=True), install))
def all(ctx):
    pass
...
def clean(ctx):
    with ctx.cd(_PROJECT_DIR):
        for d in (".venv", "build", "dist", "src/*/*.pyc", "src/cloc.egg-info", "tests/*/*.pyc"):
